...
if (isset($_SESSION['admin']) && !isset($_SESSION['usuario'])) {
   ...
   if($usuario['token'] == 7 && $usuario['token2'] == 7){
         $sql_ordens ="SELECT * FROM ordens WHERE requisitante = $ID OR coordenador = '$ID'  AND Status = '3' OR Status = '8' AND resebido = '0' AND histCoo = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;
    }elseif( $usuario['token'] == 5 || $usuario['token2'] == 5 ){
        $sql_ordens ="SELECT * FROM ordens WHERE requisitante = $ID OR direcao = '$ID' AND Status = '4' AND resebido = '0' AND histDir = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;

    }elseif($usuario['token'] == 13 || $usuario['token2'] == 13){

        $sql_ordens ="SELECT * FROM ordens WHERE requisitante = $ID OR Status = '5' AND resebido = '0' AND histAlm = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;
    }elseif ($usuario['token'] == 11 || $usuario['token2'] == 11){

        $sql_ordens ="SELECT * FROM ordens WHERE requisitante = $ID OR Status = '1' AND resebido = '0' AND histPro = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;

    }elseif($usuario['token'] == 12 || $usuario['token2'] == 12){

        $sql_ordens ="SELECT * FROM ordens WHERE requisitante = $ID OR Status = '0' OR Status = '2' OR Status = '7' AND resebido = '0' AND histCom = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;

    }elseif ($usuario['token'] == 3 || $usuario['token2'] == 3){

        $sql_ordens ="SELECT * FROM ordens WHERE requisitante = '$ID'AND resebido = '0' AND histUsu = 0 ORDER BY `ordens`.`Data` DESC";
        $query_ordens = $mysql->query($sql_ordens) or die($mysql->error);
        $num_ordens = $query_ordens->num_rows;
